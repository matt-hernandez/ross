...
@Component({
  selector: 'g[i-ng-axis-cross-section]',
  templateUrl: './axis-cross-section.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class AxisCrossSectionComponent
  extends applyMixins([Stylable, DataSubscribable])
  implements OnChanges, BaseChartComponent {
  /**
  * The specific style for the bar.
  */
  @Input() style: AxisElementsTheme['crossSection'];
  @Input() x: number;
  @Input() y: number;
  @Input() axisType: AxisType;
  @ViewChild('animatingNode') animatingNode: ElementRef;

  rootStylePath = 'axisElements.crossSection';
